...
assmLen - total length of all contigs (in bp)

meanLen - mean length of the contigs (in bp)

medLen - median length of the contig (in bp)
...
rdUsagePc - percentage of input reads used in the assembly

alignPerc - percentage of assembly sequence that align to a \Sexpr{refName} transcript using Blat with default parameters

blatHit - number of contigs that aligned 90\% of their sequence to a \Sexpr{refName} transcript
...
<<tableMe,results=tex,echo=FALSE>>=
...
limitedTable<-as.data.frame(myDataFrame[,c("kmer","cvCut","expCov","ctgs","totalCoverage","N50","meanLgth","tiles","rdUsagePc",
...
names(limitedTable)[which(names(limitedTable)=='medianLgth')]<-'medLen'
names(limitedTable)[which(names(limitedTable)=='meanLgth')]<-'meanLen'
names(limitedTable)[which(names(limitedTable)=='totalCoverage')]<-'assmLgth'
names(limitedTable)[which(names(limitedTable)=='uniqueHits')]<-'uniqueHit'
