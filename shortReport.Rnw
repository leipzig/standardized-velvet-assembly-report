...
if("bpAligned" %in% colnames(myDataFrame)){limitedTable["alignPerc"]<-myDataFrame['bpAligned']*100/myDataFrame['totBP']
                                         limitedTable["blatHit"]<-myDataFrame$blatHit
                                       }
...
myXtableOne<-xtable(underTab[order(underTab$kmer,underTab$cvCut,decreasing=FALSE),],type=tex,caption="Assembly Parameter Perumutations")
#myXtableTwo<-xtable(overTab[order(overTab$kmer,overTab$cvCut,decreasing=FALSE),],type=tex,caption="Assembly Parameter Perumutations")
@ 
